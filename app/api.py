...
def build_api_router(container: ServiceContainer) -> APIRouter:
    ...
    async def list_receipts(telegram_id: int, session: AsyncSession = Depends(get_session)) -> list[ReceiptView]:
    ...
    async def budgets(telegram_id: int, session: AsyncSession = Depends(get_session)) -> list[BudgetProgress]:
        user = await container.user_repo(session).by_telegram_id(telegram_id)
        if user is None:
            raise HTTPException(status_code=404, detail="User not found")
    ...
    async def mydata(telegram_id: int, session: AsyncSession = Depends(get_session)) -> MyDataExport:
        user = await container.user_repo(session).by_telegram_id(telegram_id)
        if user is None:
            raise HTTPException(status_code=404, detail="User not found")
